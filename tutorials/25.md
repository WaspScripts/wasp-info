...
We want to save this data into a constant, as we do not want the value to ever change. We will now discuss the type TRSMapChunk. There are two variables within: Chunk and Planes. Chunk is a box made from two coordinates that stores the variables we just acquired, and Planes contains the different levels in terms of height. We are not going up or down a ladder/staircase so we only have to worry about plane 0 (the overworld/ground level). We will set up our TRSMapChunk as a constant, and we will set the values on the same line. We will first put in our pair of coordinates followed by the planes we want to use, which in this case is only the singular plane 0. This will look like this:
...
begin
  Map.SetupChunkEx(MYCHUNK.Chunk, [0], 8);
end. 
```
